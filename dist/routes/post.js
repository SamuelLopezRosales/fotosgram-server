...
// crear post
postRoutes.post('/', [autentificacion_1.verificaToken], (req, res) => {
    const body = req.body;
...
postRoutes.post('/upload', [autentificacion_1.verificaToken], (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    if (!req.files) {
    ...
    // este es el file o el archivo
    const file = req.files.image;
    if (!file) {
        console.log("No se subio ningun archivo - image");
        return res.status(400).json({
            ok: false,
    ...
    if (!file.mimetype.includes('image')) {
        console.log("Lo que subio no es una imagen");
        return res.status(400).json({
            ok: false,
